...
class TestImputerAdapter:
    ...
    def test_fallback_when_external_module_missing(self):
        """P2.4: 外部模块缺失时抛 AdapterImportError (不再静默回退)"""
        from factor_pipeline.exceptions import AdapterImportError

        # P2.4: ImputerAdapter 是 REQUIRED 依赖, mock 路径触发 AdapterImportError
...
class TestProcessingAdapter:
    ...
    def test_transform_outlier(self, extreme_outlier_data):
        """P2.4: 测试去极值变换 (使用真实 AdaptiveWinsor 模块, 需先 fit)"""
        adapter = ProcessingAdapter(process_type='outlier', method='mad')
        adapter.fit(extreme_outlier_data)
    ...
    def test_transform_standardization(self, clean_factor_data):
        """P2.4: 测试标准化变换 (使用真实 AdaptiveWinsor 模块, 需先 fit)"""
        adapter = ProcessingAdapter(process_type='standardization', method='z_score')
        adapter.fit(clean_factor_data)
        result = adapter.transform(clean_factor_data)

        # P1.3: 按列 fit 后, 每列均值应接近 0
...
class TestNeutralizerAdapter:
    ...
    def test_fallback_when_external_module_missing(self):
        """P3.2: 外部模块缺失时抛 AdapterImportError (不再静默回退)"""
        from factor_pipeline.exceptions import AdapterImportError

        # P3.2: NeutralizerAdapter 是 REQUIRED 依赖, mock 路径触发 AdapterImportError
        with pytest.raises(AdapterImportError):
            NeutralizerAdapter(
                module_path='../NonExistent',
                import_path='does.not.exist',
