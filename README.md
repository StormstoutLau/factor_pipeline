...
这套管线不是"又一个因子库",而是一台**因子测谎仪**。

它源于一个朴素的观察:量化行业充斥着"自信表演"——精美的回测曲线、复杂的模型术语、看似严谨的统计检验,但很少有人回答最根本的问题:**这个因子是真的吗?这个模型在什么条件下会失效?我应该相信它到什么程度?**

大多数因子处理框架默认"因子是有效的,我们来加工它"。这套管线反其道而行:**先诊断,再处理,持续监测**。21 维因子指纹不是装饰,是给每个因子做体检;KS 迁移检测的 BH-FDR 校正不是炫技,是诚实地处理多重比较的假阳性风险;三条差异化管道不是功能堆砌,是承认"静态因子、动态因子、混合因子需要不同的处理逻辑"。

换句话说,这不只是一套代码,而是一种**认识论立场**:不信任因子、不信任模型、不信任自己的判断,然后用统计工具精确测量"在多大程度上可以相信"。
