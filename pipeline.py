...
from .config import StepType, PipelineConfig, StepConfig
...
class PipelineOrderValidator:
    ...
    内部委托给 PipelineDAG (networkx.DiGraph) 进行依赖关系管理。
    """
    ...
    def validate(cls, steps: List[StepType], strict: bool = True) -> Tuple[bool, List[str]]:
        ...
        # 2. 检查依赖关系（委托给 DAG）
        valid, dag_errors = cls._dag.validate(steps, strict=False)  # 非严格模式先检查依赖
        # 过滤掉 "第一步" 和 "空列表" 错误（已在上面处理）
        dag_errors = [e for e in dag_errors if "第一步" not in e and "列表为空" not in e]
        ...
        # 4. 严格模式: 检查是否在预定义的标准顺序列表中
        if strict:
            _, strict_errors = cls._dag.validate(steps, strict=True)
            # 过滤掉非严格模式已有错误，只保留严格模式特有的
            strict_only = [e for e in strict_errors 
                          if "顺序警告" in e or "不在预定义" in e]
            if strict_only and len(errors) == 0:
    ...
    def _get_reason(cls, step: StepType, required: StepType) -> str:
        """获取顺序要求的原因说明（委托给 DAG）"""
        return cls._dag._get_reason(step, required)
    
    @classmethod
    def suggest_correction(cls, steps: List[StepType]) -> List[StepType]:
        """
        建议修正后的顺序（委托给 DAG 的拓扑排序）
        """
        return cls._dag.suggest(steps)
...
class FactorProcessingPipeline:
    ...
    def _execute(self, X: pd.DataFrame, **kwargs) -> PipelineResult:
        ...
        for name, step in self.steps.items():
            ...
            # 尝试从缓存读取
            cached_result = None
            input_data = current_data  # 保存 transform 前的引用，用于缓存 key
            if self.cache is not None:
                step_params = getattr(step, 'params', {})
                cached_result = self.cache.get(
                    name, "pipeline_factor", step_params, input_data
                )
            
            try:
                if cached_result is not None:
                    logger.info(f"执行步骤: {name} (缓存命中)")
                    ...
                    if self.cache is not None:
                        step_params = getattr(step, 'params', {})
                        self.cache.set(
                            name, "pipeline_factor", step_params,
