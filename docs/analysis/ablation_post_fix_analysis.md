...
## 附录: P1 StaticPipeline 中性化顺序 A/B 测试

**日期**: 2026-07-09
**方法**: 严格 A/B 对比, HAC 显著性检验, 48 股 × 1449 期 momentum_1m 因子

| 指标 | 旧顺序 (T→N) | 新顺序 (N→T) | Δ |
|------|-------------|-------------|---|
| IC mean | -0.004142 | -0.004198 | -0.000056 |
| IC std | 0.153084 | 0.152768 | — |
| LS Sharpe | -0.003086 | -0.004851 | -0.001764 |
| **p(HAC)** | — | — | **0.7989** |
| 截面 Spearman ρ | — | 0.9958 | — |

**结论: 统计不显著 (p=0.7989)**。两种顺序的 IC/Sharpe 差异在统计噪声范围内。

两种顺序的截面输出 Spearman ρ=0.9958，高度一致但不完全相同——说明 transform (Box-Cox) 和 neutralize (OLS 残差) 的操作在非线性变换下**近似可交换**，差异主要来源于 Box-Cox 对极值的非线性映射 + OLS 回归在变换空间中的性质变化。

**最终判断**:
- P1 顺序变更在统计上无显著提升
- 两种顺序基本等价，无必要强制 revert
- 保留新顺序 (neutralize→transform) 的理由是**原则性更好**（先剥离风险暴露再变换），即使统计上等价
- 但标注为 **无实证支持的优化** (p=0.80)
